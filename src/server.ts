...
import { disconnectDatabase, pingDatabase } from "./db/client.js";
...
const CORS_ORIGINS = [
  ...
  "https://doctor-dashboard-frontend-production-979f.up.railway.app"
...
async function listen(retried = false): Promise<void> {
  try {
    await app.listen({ port: PORT, host: HOST });
    app.log.info(
      { port: PORT, host: HOST, nodeEnv: process.env.NODE_ENV ?? "development" },
      "CareConnect API listening"
    );
  } catch (err) {
